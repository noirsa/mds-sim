...
m2 <- lm(log(price) ~ log(livingArea) + 
                        yearBuilt + 
...
Adding categorical variables improved performance, but the models became prone to overfitting. Because the city and county fields have high cardinality, and stepwise selection returned many NA coefficients, we decided to reduce their cardinality.
...
keep_cities <- names(tab_city[tab_city >= 20])
tab_county <- table(georgia_clean$county)

keep_counties <- names(tab_county[tab_county >= 15])
...
summary(georgia_clean$city_grp)
summary(georgia_clean$county_grp)
...
After checking for overfitting, we will study the interactions between different numerical and categorical variables.
```{r}
# Interaction: yearBuilt:bathrooms
...
summary(m3_0) # Having Little bathrooms, the values increase in 0.796 units

# Does the effect (by default 0.796) of log(livingArea) depend on the number of bathrooms?.
...
# If we have (baseline) log(livingArea):bathroomsLittle-Bathrooms -> log(livingArea): 0.796
# If we have (new) log(livingArea):bathroomsMany-Bathrooms -> log(livingArea): 0.796 + 0.844 = 1.64
# If we have (new) log(livingArea):bathroomsExcessive-Bathrooms -> log(livingArea): 0.796 + 3.30 = 4.096
...
anova(m2, m3_0) # low p-value suggests that the model m3_0 is better
...
```
Now we added all interactions to the model and then used stepwise selection to obtain the best specification.
```{r}
...
#step(m3_1)
```
After stepwise selection, the formula of best model is this
log(price) ~ log(livingArea) + bathrooms + yearBuilt + 
    poly(latitude.scaled, 2) + city_grp + county_grp + bedrooms + 
    levels_mapped + garageSpaces + hasGarage + spa + parking + 
    pool + homeType + log(livingArea):bathrooms + log(livingArea):yearBuilt + 
    log(livingArea):poly(latitude.scaled, 2) + log(livingArea):county_grp + 
    log(livingArea):bedrooms + log(livingArea):levels_mapped + 
    log(livingArea):garageSpaces + log(livingArea):hasGarage + 
    log(livingArea):spa + log(livingArea):parking + log(livingArea):pool + 
    log(livingArea):homeType, data = georgia_clean

The model obtains the following results in the training data: 

Residual standard error: 0.2884 on 4747 degrees of freedom
Multiple R-squared:  0.7837,	Adjusted R-squared:  0.7727 

```{r}
m4 = lm(log(price) ~ log(livingArea) + bathrooms + yearBuilt + 
    poly(latitude.scaled, 2) + city_grp + county_grp + bedrooms + 
    levels_mapped + garageSpaces + hasGarage + spa + parking + 
    pool + homeType + log(livingArea):bathrooms + log(livingArea):yearBuilt + 
    log(livingArea):poly(latitude.scaled, 2) + log(livingArea):county_grp + 
...
# -> with graphs and / or suitable tests to verify model assumptions.
par(mfrow=c(2,2))
plot(m4) 
par(mfrow=c(1,1))

library(ggplot2)
pred <- fitted(m4); obs <- model.response(model.frame(m4))  
...
library(effects)
plot(Effect(c("livingArea", "yearBuilt"), m4), multiline=TRUE, ci.style="bars")
plot(Effect(c("livingArea", "bathrooms"), m4), multiline=TRUE, ci.style="bars")

plot(Effect(c("livingArea", "parking"), m4), multiline=TRUE, ci.style="bars")
