...
Up to this points, all variables in the dataset have been explored. In terms of missingness, no missing data are found in the dataset. Although the proportions of categories in some of the variables are just too exaggerated, it might yield better results if we removed those columns. If we rank the numeric variables in terms of number of outliers, the following list is obtained, starting with the most outliers: longitude (97 extreme outliers), latitude (82 extreme outliers), pricePerSquareFoot (56 extreme outliers), price (43 extreme outliers), bathrooms (35 extreme outliers), bedrooms (18 extreme outliers), yearBuilt (16 extreme outliers), livingArea (9 extreme outliers, we are ignoring livingAreaValue and buildingArea, since they have the same values), zipcode (331 mild outliers) and garageSpaces (5 mild outliers).
 
**Univariate Analisis**

Now we will study the univariate outliers of the aforementioned variables. Firstly, the number of univariate outliers per individual are counted and added in a new variable called ‘univ_outl_count’. Looking at the 3 individuals with the most univariate outliers (6) it can be concluded that they are all old, one of them is not that old though, but the other two have more than 100 yers of history. All these houses are costly and have unusually high number of bathrooms and bedrooms. There is a house which has garage but it has no spaces in it and there is another house which has no stories (levels = 0). A correlation matrix confirms this as it shows a positive correlation (0.32) to the price (so the more expensive the house, the more univ outliers), it also shows a positive correlation to zipcode (0.46), livingArea (0.37), number of bathrooms (0.27) and number of bedrooms (0.21). These variables are also highly correlated with price.
...
```{r}
# pricePerSquareFoot
georgia$univ_outl_count[Boxplot(georgia$pricePerSquareFoot, id = list(n=Inf))] = georgia$univ_outl_count[Boxplot(georgia$pricePerSquareFoot, id = list(n=Inf))] +1
```
...
```{r}
# zipcode
georgia$univ_outl_count[Boxplot(georgia$zipcode, id = list(n=Inf))] = georgia$univ_outl_count[Boxplot(georgia$zipcode, id = list(n=Inf))] +1
```
...
```{r}
# There are observations with 6 outliers in those columns
max(georgia$univ_outl_count)
# Print those individuals
georgia[which(georgia$univ_outl_count == 6),]
```

```{r}
# We are going to a create a correlation matrix of those columns
georgia_interest = georgia[,c(13,14,17,19,20,21,25,27,28,31,40)]
...
# Limit the y and x axis
plot(res.out$md, res.out$rd,
     xlab = "Mahalanobis Distance",
     ylab = "Robust Distance",
     xlim = c(0, 20), # set x-axis limit
     ylim = c(0, 70)) # set y-axis limit
abline(h = res.out$cutoff, col = "red", lwd = 2)
abline(v = res.out$cutoff, col = "red", lwd = 2)

```

```{r}
# Summary of only outliers
summary(georgia[which((res.out$md > res.out$cutoff)&(res.out$rd > res.out$cutoff)),])
```
