...
llres <- which(abs(rstudent(m1_1))>3); llres
# rstudent residuals are a type of residuals that follow a t-student distribution
# In a t-student distribution, the probability of value to be higher than 3 is very small.
georgia_clean[llres,] # these are the influential observations found using llres

# A priori influential observations:
# Check which observations have a leverage higher than 3*p/n
llhat <- which(hatvalues(m1_1)>3*length(coef(m1_1))/nrow(df)); llhat
# No observations have a leverage higher than 3*p/n

# Check influential observations and remove them from the dataset
llcoo <- Boxplot(cooks.distance(m1_1), id=list(n=10,labels=row.names(georgia_clean)))
influencePlot(m1_1) # the ids of influential observations also appear in llcoo
ll <- which(row.names(georgia_clean) %in% llcoo); ll # find row ids of them
georgia_clean <- georgia_clean[-ll,] # remove them from the dataset
```
...
Before validate model we check influential value
The influence plot shows most cases with low leverage; a small set exhibit elevated Cook’s D ( 582,583,1141,1921,2657,3502,3907,4471 ). we remove the flagged cases and refit the model on the cleaned sample.



```{r}
influential <- influencePlot(m4_2,pch=19,id=list(method="noteworthy",n=3)) # we see NaN in Cook's D value because there are some influential values with hat value 1.
...
georgia_clean <- georgia_clean[-c(idx),]
...
influencePlot(m4_3,pch=19,id=list(method="noteworthy",n=3)) # After remove these influential values we can see upper bound is 0.0159
```
